...
public class WikiResolver extends MessageResolver {
    ...
    static {
        ...
        SUPPORT_WIKIS.put("moe", new WikiInfo("https://zh.moegirl.org.cn/api.php?"));
    ...
    public boolean resolveInternal(String key, MessageInfo info) {
        ...
        KoishiBotMain.INSTANCE.executor.execute(() -> {
            ...
            try {
                if (splits.length == 1 || !SUPPORT_WIKIS.containsKey(splits[0].toLowerCase(Locale.ROOT)))
                    requestWikiPage("mcw", finalKey, info);
                else
                    requestWikiPage(splits[0], splits[1], info);
            } catch (Exception e) {
                ErrorRecord.enqueueError("wiki", e);
    ...
    private static void requestWikiPage(String namespace, String title, MessageInfo info) throws Exception {
        WikiInfo wiki = SUPPORT_WIKIS.get(namespace);
        ...
        if (page.isSearched) {
            ...
            info.sendMessageAwait(chain, (sourceData, newInfo) -> {
                try {
                    ...
                    ErrorRecord.enqueueError("wiki.re_search", e);
                    MessageChain newChain = MessageUtils.newChain(
                            new QuoteReply(info.data),
                            new PlainText("调用API产生了错误：" + e.getMessage())
                    );
                    info.sendMessageWithQuote(newChain);
                }
            });
        } else {
            if (page.redirected)
    ...
    public static void main(String[] args) throws IOException {
        WikiInfo wiki = SUPPORT_WIKIS.get("mcw");
        PageInfo page = wiki.parsePageInfo("末影水晶", 0);
        System.out.println();
    }
}
