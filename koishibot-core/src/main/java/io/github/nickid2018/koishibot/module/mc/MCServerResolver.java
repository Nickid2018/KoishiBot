...
import io.github.nickid2018.koishibot.util.FuncUtils;
            ...
            if (je)
                try {
                    ...
                    Optional<ImageMessage> favicon = JsonUtil.getString(json, "favicon")
                            ...
                            .map(FuncUtils.rethrow(bytes -> {
                                File faviconFile = TempFileSystem.createTmpFile("favicon", ".png");
                                try (FileOutputStream fos = new FileOutputStream(faviconFile)) {
                                    fos.write(bytes);
                                }
                                return faviconFile.toURI().toURL();
                            }))
                            .map(environment::newImage);
