        ...
        try {
            ...
            if (authSequence.containsKey(state)) {
                ...
                if (refreshTokenEnabled) {
                    String refreshToken = JsonUtil.getStringOrNull(object, "refresh_token");
                    long expiredTime = JsonUtil.getIntOrZero(object, "expires_in") * 1000L + System.currentTimeMillis();
                    AuthenticateToken token = new AuthenticateToken(accessToken, expiredTime, refreshToken, authData.second());
                    dataReader.getData().put(authData.first(), token);
                    dataReader.saveData();
                }

                authData.third().accept(accessToken);
            } else {
                String fail = """
                        <!DOCTYPE HTML>
                        <head>
